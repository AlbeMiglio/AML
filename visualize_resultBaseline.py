...
def compute_pinhole_translation(bbox, intrinsics, real_diameter):
    """Compute T(X, Y, Z) using YOLO BBox and real diameter."""
    x, y, w, h = bbox
    ...
    # Depth Z depends on real diameter and YOLO box size
...
def preprocess_for_resnet(img, bbox):
    """Crop and prepare image for ResNet-50."""
...
def main():
    ...
    pose_model.load_state_dict(torch.load(RESNET_PATH, map_location=DEVICE), strict=False)
    pose_model.eval()
    ...
    with torch.no_grad():
        ...
        for idx in indices:
            ...
            pred_quat, _ = pose_model(input_resnet.to(DEVICE))
