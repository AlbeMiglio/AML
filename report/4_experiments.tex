...
\paragraph{Training}
All pose models are trained with Adam (lr $10^{-4}$, weight decay $10^{-4}$), batch size 64, ADD loss (Eq.~\ref{eq:add_loss}), \texttt{ReduceLROnPlateau} scheduling, and early stopping on validation ADD (patience 30, epoch cap 400). The optimal hyperparameter values were determined via a Grid Search on the validation set. Photometric augmentation only (Sec.~\ref{sec:method_fusion}).
...
Three observations. First, direct absolute regression (6.8\%) performs \emph{below} the learning-free anchor reference (10.4\%): the network cannot even recover the information that the depth median already provides, confirming that the failure is in the formulation of the target rather than in the amount of data. Second, anchoring the target transforms the same model into an accurate estimator (95.2\%), with the mean error dropping from 69.7 to 9.1\,mm. Across three independent training seeds, the \texttt{norm} variant scores $94.9 \pm 0.2$\%, indicating that the result is not seed-dependent.
