...
Instead of relying only on the standard mAP@50 metric, we focus on the stricter mAP@50-95 (mean AP computed over IoU thresholds from 0.5 to 0.95 in steps of 0.05). Getting a tight bounding box matters a lot for the downstream pose estimation: an oversized crop brings in background noise that can corrupt the rotation features extracted by the ResNet backbone.
...
Cropping the object throws away two strong cues: how large it appeared and where it sat in the frame. Both constrain its position in space, and neither survives the resize to a fixed $224\times224$. We restore them explicitly as an 8D vector:
\begin{equation}
...
A two-layer MLP ($8 \rightarrow 128 \rightarrow 64$, ReLU) maps $\mathbf{m}$ to $\mathbf{f}_{\text{meta}} \in \R^{64}$. Every component is normalized so that focal lengths of several hundred pixels do not dominate the gradients of coordinates below one. The intrinsics matter as much as the box: a large box means a near object only once the focal length is known, and the convolutional branches never see $\mathbf{K}$.
