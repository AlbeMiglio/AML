...
Early deep learning methods for 6D pose estimation directly regress the 3D translation and orientation from RGB images. PoseCNN~\cite{xiang2018posecnn} introduced an end-to-end architecture that localizes objects and estimates poses via a regression head. To bypass the difficulty of direct translation regression, PVNet~\cite{peng2019pvnet} predicts 2D keypoint locations through a pixel-wise voting scheme and recovers the 6D pose by a subsequent PnP step, achieving strong robustness to occlusion. More recently, GDR-Net~\cite{wang2021gdrnet} closes the gap between direct regression and PnP-based pipelines by exploiting dense 2D--3D correspondences within a fully differentiable formulation. Despite their effectiveness, all RGB-only approaches inherently suffer from \emph{depth ambiguity}: predicting the Z-axis translation relies heavily on the apparent size of the bounding box, an approximation that fails drastically for elongated or asymmetric objects viewed from varying angles -- as we empirically demonstrate in Sec.~\ref{sec:exp_results}.

\subsection{RGB-D Fusion for Pose Estimation}
\label{sec:related_rgbd}

To address the fundamental limitations of monocular vision, researchers have leveraged depth sensors to provide explicit geometric information. DenseFusion~\cite{wang2019densefusion} proposed a pixel-wise dense fusion network that extracts features from RGB images and 3D point clouds separately before fusing them via a per-pixel heterogeneous concatenation; each observed 3D point predicts a pose relative to itself, which implicitly grounds the translation estimate in the measured geometry. Subsequent keypoint-based RGB-D methods pushed accuracy further: PVN3D~\cite{he2020pvn3d} votes for 3D keypoints from the fused point cloud, and FFB6D~\cite{he2021ffb6d} introduces bidirectional fusion at every encoding layer. Both, however, augment the scarce real LineMod training set with tens of thousands of rendered synthetic images per object, and rely on dense per-point processing.

Our work proposes a ``Global Fusion'' alternative, extracting 1D global latent vectors from both modalities through independent backbones and fusing them via concatenation in a shared latent space. We retain the key geometric insight of per-point methods --- predictions should be expressed relative to observed 3D evidence --- but apply it once, globally, through a single back-projected anchor point, keeping the architecture lightweight and the training data real-only.
